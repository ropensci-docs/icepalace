...
of R
([`rversions::r_release()`](https://r-hub.github.io/rversions/reference/r_release.html)).
If `type` includes `win.binary` or `mac.binary` (by default, `type`
includes source and both kinds of binaries), and there are no binary for
the R version, the function will warn.
